...
File.prototype.generateBatches = function(done1) {
  ...
  async.each(this._batches, function(batch, done2) {
    ...
    var finalRows = rows + addendaCount;
    self.control.addendaCount.value = addendaCount * 2;
    self.control.blockCount.value = utils.getNextMultiple(finalRows, 10) / 10;
      ...
      self.generateBatches(function(batchString, rows) {
        self.generateControl(function(controlString) {
          ...
          self.generatePaddedRows(paddedRows, function(paddedString) {
            var str = headerString + utils.newLineChar() + batchString + controlString + paddedString;
            cb && cb(undefined, str);
            resolve(str);
          });
        });
